...
import click
import numpy as np
from .crepe_notes import process, parse_f0, run_pitch_tracker

@click.command()
@click.option('--output-label', default=None, help='Label for output filename. Defaults to "{pitch_tracker}.transcription"')
...
@click.option('--pitch-tracker', type=click.Choice(['crepe', 'pesto', 'penn', 'torchcrepe'], case_sensitive=False), default='crepe', help='Pitch tracking backend to use')
@click.argument('audio_path', type=click.Path(exists=True, path_type=pathlib.Path))
@click.help_option()
def main(f0, audio_path, output_label, sensitivity, min_duration, min_velocity, disable_splitting, tuning_offset, use_smoothing, use_cwd, save_analysis_files, pitch_tracker):
    """crepe_notes - get discrete MIDI notes from CREPE pitch estimates"""
    default_f0_path = audio_path.with_suffix(f'.{pitch_tracker}.f0.csv')
    legacy_f0_path = audio_path.with_suffix('.f0.csv')
    if f0 is not None:
        frequency, confidence = parse_f0(f0)
    elif default_f0_path.exists():
        frequency, confidence = parse_f0(default_f0_path)
    elif legacy_f0_path.exists():
        frequency, confidence = parse_f0(legacy_f0_path)
    else:
    ...
    process(frequency, confidence, audio_path, output_label=output_label, sensitivity=sensitivity, use_smoothing=use_smoothing,
            ...
            tuning_offset=tuning_offset, save_analysis_files=save_analysis_files, pitch_tracker=pitch_tracker)
